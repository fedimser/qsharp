...
impl Lowerer {
    ...
    fn lower_stmt(&mut self, stmt: &crate::ast::Stmt) -> Option<super::ast::Stmt> {
        let kind = match &*stmt.kind {
            ...
            crate::ast::StmtKind::Continue(stmt) => {
                super::ast::StmtKind::Continue(self.lower_continue_stmt(stmt)?)
            }
    ...
    fn lower_unary_op_expr(&mut self, expr: &crate::ast::UnaryOpExpr) -> Option<super::ast::Expr> {
        match expr.op {
            ...
            crate::ast::UnaryOp::NotL => {
                if let crate::ast::ExprKind::Lit(lit) = expr.expr.kind.as_ref() {
                    if let crate::ast::LiteralKind::Bool(value) = lit.kind {
                        let kind = super::ast::LiteralKind::Bool(!value);
                        let ty = Type::Bool(true);
                    ...
                    self.push_unsupported_error_message(
                        "Logical not on non-bool literal",
                        expr.expr.span,
                    );
                    return None;
                }

                let target_ty = Type::Bool(true);
                let expr =
    ...
    fn lower_assign(&mut self, stmt: &crate::ast::Assign) -> Option<super::ast::Assign> {
        self.push_unimplemented_error_message("assign stmt", stmt.span);
        None
    }

    fn lower_assign_op(&mut self, stmt: &crate::ast::AssignOp) -> Option<super::ast::AssignOp> {
    ...
    fn lower_break(&mut self, stmt: &crate::ast::BreakStmt) -> Option<super::ast::BreakStmt> {
    ...
    fn coerce_literal_expr_to_type(
        ...
        if types_equal_except_const(ty, &rhs.ty) {
            // we know it is the rhs as it is a literal, but we need to relax the const
            // and use the lhs ty
            if rhs.ty.is_const() {
                return Some(super::ast::Expr {
                    span: rhs.span,
        ...
        if matches!(lhs_ty, Type::Bit(..)) {
            if let super::ast::LiteralKind::Int(value) = literal.kind {
                // can_cast_literal_with_value_knowledge guarantees that value is 0 or 1
                return Some(super::ast::Expr {
                    span,
                    kind: Box::new(super::ast::ExprKind::Lit(super::ast::Lit {
                        span: literal.span,
                        kind: super::ast::LiteralKind::Int(value),
                    })),
                    ty: lhs_ty,
                });
            } else if let super::ast::LiteralKind::Bool(value) = literal.kind {
                return Some(super::ast::Expr {
                    span,
                    kind: Box::new(super::ast::ExprKind::Lit(super::ast::Lit {
                        span: literal.span,
                        kind: super::ast::LiteralKind::Int(i64::from(value)),
                    })),
                    ty: lhs_ty,
                });
            }
        ...
        if is_int_to_bit_array {
            if let super::ast::LiteralKind::Int(value) = literal.kind {
                ...
                return Some(super::ast::Expr {
                    ...
                    kind: Box::new(super::ast::ExprKind::Lit(super::ast::Lit {
                        span: literal.span,
                        kind: super::ast::LiteralKind::Bitstring(value, size),
                    })),
                    ty: lhs_ty,
                });
            }
        ...
        let result = match (&lhs_ty, &rhs_ty) {
                ...
                if let super::ast::LiteralKind::Int(value) = literal.kind {
                    if let Some(value) = safe_i64_to_f64(value) {
                        return Some(super::ast::Expr {
                            ...
                            kind: Box::new(super::ast::ExprKind::Lit(super::ast::Lit {
                                span: literal.span,
                                kind: super::ast::LiteralKind::Float(value),
                            })),
                            ty: lhs_ty,
                        });
                    }
            ...
            (Type::Float(..), Type::Float(..)) => {
                if let super::ast::LiteralKind::Float(value) = literal.kind {
                    return Some(super::ast::Expr {
                        span,
                        kind: Box::new(super::ast::ExprKind::Lit(super::ast::Lit {
                            span: literal.span,
            ...
            (Type::Complex(..), Type::Complex(..)) => {
                if let super::ast::LiteralKind::Complex(real, imag) = literal.kind {
                    return Some(super::ast::Expr {
                        span,
                        kind: Box::new(super::ast::ExprKind::Lit(super::ast::Lit {
                            span: literal.span,
                            kind: super::ast::LiteralKind::Complex(real, imag),
                        })),
                        ty: lhs_ty,
                    });
                }
            ...
            (Type::Complex(..), Type::Float(..)) => {
                if let super::ast::LiteralKind::Float(value) = literal.kind {
                    return Some(super::ast::Expr {
                        ...
                        kind: Box::new(super::ast::ExprKind::Lit(super::ast::Lit {
                            span: literal.span,
                            kind: super::ast::LiteralKind::Complex(value, 0.0),
                        })),
                        ty: lhs_ty,
                    });
                }
                ...
                match &literal.kind {
                    ...
                    super::ast::LiteralKind::BigInt(value) => {
                        ...
                        return Some(super::ast::Expr {
                            ...
                            kind: Box::new(super::ast::ExprKind::Lit(super::ast::Lit {
                                span: literal.span,
                                kind: super::ast::LiteralKind::BigInt(value.clone()),
                            })),
                            ty: lhs_ty,
                        });
                    }
    ...
    fn lower_negated_literal_as_ty(
        ...
        let (kind, ty) = (match &lit.kind {
            ...
            crate::ast::LiteralKind::Duration(_, _) => {
                self.push_unsupported_error_message("negated duration literal expressions", span);
                None
            }
            crate::ast::LiteralKind::String(_) => {
        ...
        let lit = super::ast::Lit {
            span: lit.span,
            kind,
        };
        let expr = super::ast::Expr {
            span,
            kind: Box::new(super::ast::ExprKind::Lit(lit.clone())),
            ty,
        };
        if let Some(target_ty) = target_ty {
            return self.coerce_literal_expr_to_type(&target_ty, &expr, &lit);
        }
        Some(expr)
    }
