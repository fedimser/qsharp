...
pub(crate) type Result<T> = std::result::Result<T, crate::semantic::error::Error>;
pub(crate) trait Parser<T, S>: FnMut(&S) -> Result<T>
where
    S: AsRef<str>,
{
}

impl<T, S: AsRef<str>, F: Fn(&S) -> Result<T>> Parser<T, S> for F {}
